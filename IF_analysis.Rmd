...
## Notes/Changes
Add the frequency chart to question 2
...
full_data <- taxa %>%
  inner_join(water_quality1,
             by=c("site_name"="site_name", "transect_number"="transect_number", "condition"="condition", "date"="date", "analogue"="analogue"),
             suffix=c("_t", "_wq"))

full_data$location <- paste(full_data$site_name, full_data$transect_number)
```
...
ggplot(sum_sites, aes(x=factor(site_name), y=total, fill=factor(condition)))+
  geom_bar(stat="identity")+
  scale_x_discrete(limits=c("j1", "j3", "cc", "ccr", "ar", "hb"))+
  labs(x="Analogue", y="Individuals Found", fill="Condition")

ggplot(sum_sites, aes(x=factor(site_name), y=biodiversity, fill=factor(condition)))+
  geom_bar(stat = "identity")+
  scale_x_discrete(limits=c("j1", "j3", "cc", "ccr", "ar", "hb"))+
  scale_y_continuous(labels=scales::percent, limits=c(0, 0.5))+
  labs(x="Site Names", y="Biodiversity Ratio", fill="Condition")
```
...
```{r}
ggplot(sum_sites, aes(x=mean_cover, y=total))+
  geom_point(aes(color=condition, shape=site_name), size=3)+
  scale_x_continuous(labels=scales::percent)+
  labs(x="Fe Floc Percent Cover", y="Total Individuals Found", color="Condition", shape="Sites")
  

ggplot(sum_sites, aes(x=mean_cover, y=count_taxa))+
  geom_point(aes(color=condition, shape=site_name), size=3)+
  scale_y_continuous(limits=c(0, 10))+
  scale_x_continuous(limits=c(0, 1), labels=scales::percent)+
  labs(x="Fe Floc Percent Cover", y="Family Richness", color="Condition", shape="Sites")

#ggplot(full_data, aes(x=location, y=n_distinct(taxa), fill=condition))+
#  geom_col(position="dodge")

ggplot(tally(group_by(full_data, location, taxa)),
       aes(x=reorder(location, -n), y=n, fill=taxa))+
  geom_bar(stat="identity")+
  coord_flip()+
  labs(x="Site Names", y="Family Richness", fill="Taxa")
```
